...
export function TauriAppBar() {
  ...
  const menuBar: MenuBarItem[] = [
    ...
    },
    {
      ...
      items: [
        ...
        { separator: true },
        { label: "Zoom In", icon: ZoomIn, shortcut: "⌘+", action: () => {
          document.body.style.zoom = `${(parseFloat(document.body.style.zoom || "1") * 1.1)}`
        }},
        { label: "Zoom Out", icon: ZoomOut, shortcut: "⌘-", action: () => {
          document.body.style.zoom = `${(parseFloat(document.body.style.zoom || "1") * 0.9)}`
        }},
    ...
    },
    {
      label: "Go",
      items: [
        { label: "Home", icon: Home, shortcut: "⌘1", action: () => router.push("/") },
        { label: "Blog", icon: BookOpen, shortcut: "⌘2", action: () => router.push("/blog") },
        { label: "Research", icon: FlaskConical, shortcut: "⌘3", action: () => router.push("/research") },
        { label: "Contact", icon: Mail, shortcut: "⌘4", action: () => router.push("/contact") },
        { separator: true },
        { label: "Back", shortcut: "⌘[", action: () => window.history.back() },
        { label: "Forward", shortcut: "⌘]", action: () => window.history.forward() },
      ]
    },
    {
      label: "Tools",
  ...
  return (
    <div className="flex h-10 items-center border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 px-3">
