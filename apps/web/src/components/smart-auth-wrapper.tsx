...
function CrossmintAuthWithFallback() {
  ...
  useEffect(() => {
    const handleError = (event: ErrorEvent) => {
      ...
      if (event.filename?.includes('crossmint') || 
          event.error?.stack?.includes('crossmint') ||
          event.message?.toLowerCase().includes('url')) {
        console.warn("Crossmint error detected, switching to fallback:", event.error)
        setHasError(true)
      }
    }

    const handleUnhandledRejection = (event: PromiseRejectionEvent) => {
      if (event.reason?.stack?.includes('crossmint') ||
          event.reason?.message?.toLowerCase().includes('url')) {
        console.warn("Crossmint promise rejection detected, switching to fallback:", event.reason)
        setHasError(true)
      }
    }
