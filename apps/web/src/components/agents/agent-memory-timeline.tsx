...
export function AgentMemoryTimeline({ userId, agentId, className }: AgentMemoryTimelineProps) {
  ...
  const getTypeColor = (type: string) => {
    switch (type) {
      case 'conversation': return 'text-blue-400 border-blue-500/50 bg-blue-500/10'
      case 'learning': return 'text-green-400 border-green-500/50 bg-green-500/10'
      case 'reflection': return 'text-purple-400 border-purple-500/50 bg-purple-500/10'
      case 'context': return 'text-orange-400 border-orange-500/50 bg-orange-500/10'
      case 'preference': return 'text-pink-400 border-pink-500/50 bg-pink-500/10'
  ...
  const getImportanceIcon = (importance?: number) => {
    ...
    if (importance >= 8) return <Star className="h-3 w-3 text-yellow-400 fill-current" />
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {agent.name} • {filteredMemories.length} memories in {timeRange === 'all' ? 'total' : `last ${timeRange}`}
            </p>
          </div>
        </div>
      ...
      <div className="space-y-6">
        {Object.keys(groupedMemories).length === 0 ? (
          <GlassCard className="p-8 text-center">
            ...
            <h4 className="font-semibold mb-2">No Memories Found</h4>
            <p className="text-sm text-muted-foreground">
                ...
                <div className="space-y-2 ml-4">
                  {dayMemories
                    ...
                    .map((memory) => {
                      ...
                      return (
                        <div key={memory._id} className="flex gap-3">
                          {/* Timeline indicator */}
                          <div className="flex flex-col items-center">
                            ...
                            <div className="w-px bg-white/10 h-4 mt-2"></div>
                          </div>

                          {/* Memory content */}
                          <GlassCard className="flex-1 p-3 hover:bg-white/5 transition-colors">
                            <div className="flex items-start justify-between mb-2">
                              <div className="flex items-center gap-2">
                                <Badge variant="outline" className={cn("text-xs", typeColor.split(' bg-')[0])}>
                                  {memory.type}
                                </Badge>
                            ...
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-2">
                                {memory.metadata?.tags && (
                                  <div className="flex gap-1">
                                    {memory.metadata.tags.slice(0, 2).map((tag, index) => (
