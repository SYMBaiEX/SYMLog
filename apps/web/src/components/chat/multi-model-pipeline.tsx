...
export function MultiModelPipeline({
  ...
  const initializePipeline = async () => {
    ...
    const steps: PipelineStep[] = pipelineConfig.pipeline.map((modelId, index) => ({
      model: modelId,
  ...
  const getRoleForModel = (modelId: string, taskType: string, stepIndex: number): string => {
    switch (taskType) {
      case 'research':
  ...
  const executeStep = async (stepIndex: number) => {
    ...
    try {
      ...
      const stepInput = stepIndex === 0 ? input : results[stepIndex - 1]?.output || input
      
      // Mock processing time based on model complexity
      const processingTime = step.model.includes('o3') || step.model.includes('o4') ? 3000 : 
                           step.model.includes('nano') ? 1000 : 2000
      ...
      // Store result
      const newResult = { stepIndex, model: step.model, output: mockOutput, duration }
    ...
    try {
      ...
      if (!isPaused) {
        ...
        const finalResult = {
          ...
          steps: pipeline,
          results,
          totalDuration: pipeline.reduce((sum, step) => sum + (step.duration || 0), 0)
        }
  ...
  const generateMockOutput = (model: string, role: string, input: string): string => {
    ...
    const outputs = {
      ...
      'Deep Analysis': `Performing comprehensive analysis of the gathered information, identifying patterns, relationships, and key insights.`,
      'Synthesis & Summary': `Synthesizing all findings into a coherent summary with actionable recommendations and conclusions.`,
      'Technical Analysis': `Analyzed code structure, identified potential issues, and evaluated adherence to best practices.`,
      'Logic Review': `Reviewed algorithmic logic, identified edge cases, and verified correctness of implementation.`,
      'Content Generation': `Generated high-quality content based on the requirements and context provided.`
    }

    return outputs[role] || `Processed using ${model}: ${input.slice(0, 100)}...`
  }
    ...
    <GlassCard className={cn("p-6", className)}>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-semibold">Multi-Model Pipeline</h3>
            <p className="text-sm text-muted-foreground">
              {taskType.replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase())} • {totalSteps} steps
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          {!isRunning && completedSteps === 0 && (
            <GlassButton onClick={runPipeline} className="gap-2">
          ...
          {isRunning && (
            <GlassButton onClick={pausePipeline} variant="outline" className="gap-2">
          ...
          {isPaused && (
            <GlassButton onClick={resumePipeline} className="gap-2">
              <Play className="h-4 w-4" />
              Resume
            </GlassButton>
          )}
          ...
          return (
            <div key={index} className="relative">
              ...
              <div className={cn(
                ...
                isActive && "bg-periwinkle/10 border border-periwinkle/20",
                ...
                <div className={cn(
                  ...
                  step.status === 'running' && "bg-blue-500/20 animate-pulse",
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <Badge variant="outline" className="text-xs">
                      {step.model.replace(/^gpt-/, 'GPT-').replace(/^o(\d)/, 'o$1')}
                    </Badge>
                    <StatusIcon className={cn("h-4 w-4", getStatusColor(step.status))} />
                    {step.duration && (
                ...
                {index < pipeline.length - 1 && step.status === 'completed' && (
                  <ArrowRight className="h-4 w-4 text-green-400 flex-shrink-0 mt-4" />
                )}
              </div>
            </div>
          )
      ...
      {completedSteps === totalSteps && totalSteps > 0 && (
        <div className="mt-6 p-4 bg-green-500/10 border border-green-500/20 rounded-lg">
          ...
          <div className="text-sm text-muted-foreground">
            ...
            {pipeline.reduce((sum, step) => sum + (step.duration || 0), 0)}ms total duration.
          </div>
        </div>
      )}
    </GlassCard>
