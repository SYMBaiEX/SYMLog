import { generateText, streamText, stepCountIs, type StopCondition } from 'ai'
import { getAIModel } from './providers'
import { artifactTools } from './tools/artifact-tools'
import type { CoreMessage, CoreToolMessage, ToolCallPart, ToolResultPart } from 'ai'
...
export class WorkflowController {
  ...
  async executeMultiStepWorkflow(
    ...
    try {
        ...
        onStepFinish: async ({ step, stepType, toolCalls, toolResults }) => {
          ...
          for (const condition of stopConditions) {
            if (condition(workflowStep)) {
              console.log(`Workflow stopped by custom condition at step ${stepNumber}`)
              return { stop: true }
            }
          }
      ...
      const workflowError = new Error(`Workflow execution failed: ${errorMessage}`)
  ...
  async streamWorkflow(
    ...
    return streamText({
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error'
      throw new Error(`Stream workflow failed after ${this.steps.length} steps: ${errorMessage}`)
    }
  }
  ...
  async executeResearchWorkflow(
    ...
    return this.executeMultiStepWorkflow(researchPrompt, {
      ...
      stopConditions: [
        ...
        (step) => step.stepNumber > 5 && step.content?.includes('sufficient information')
      ],
  ...
  async executeAnalysisWorkflow(
    ...
    return this.executeMultiStepWorkflow(analysisPrompt, {
      ...
      stopConditions: [
        ...
        (step) => step.content?.includes('## Recommendations') && 
                  step.content?.includes('## Conclusion')
      ],
      onStepFinish: async (step) => {
        if (step.toolCalls?.some(tc => tc.toolName === 'createChartArtifact')) {
...
export function createStopCondition(
  ...
  const condition = (step: any) => {
    try {
      ...
      console.error(`Stop condition error${description ? ` (${description})` : ''}:`, error)
      return false
    }
  }
...
export const commonStopConditions = {
  ...
  afterToolExecution: (toolName: string) => 
    createStopCondition(
      step => step.toolResults?.some(tr => tr.toolName === toolName) ?? false,
