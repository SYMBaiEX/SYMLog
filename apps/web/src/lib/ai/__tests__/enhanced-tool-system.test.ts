import { describe, test, expect, beforeEach, jest, afterEach } from 'bun:test'
import { AdvancedCodeValidator } from '../code-validator'
import { ToolChoiceEnforcer, toolChoiceEnforcer } from '../tool-choice'
import { StreamingProgressManager, streamingProgressManager } from '../streaming-progress'
...
describe('Enhanced Tool System', () => {
  describe('AdvancedCodeValidator', () => {
    ...
    test('should detect security vulnerabilities', async () => {
      ...
      expect(result.securityIssues.some(issue => issue.type === 'eval')).toBe(true)
      expect(result.securityIssues.some(issue => issue.type === 'xss')).toBe(true)
    })
    ...
    test('should detect performance issues', async () => {
      ...
      const result = await validator.validateCode(inefficientCode, 'javascript')
    ...
    test('should suggest modernizations', async () => {
      ...
      expect(result.suggestions.some(s => s.type === 'modern' && s.message.includes('const or let'))).toBe(true)
      expect(result.suggestions.some(s => s.type === 'modern' && s.message.includes('template literals'))).toBe(true)
    })
    ...
    test('should repair code automatically', async () => {
      ...
      const validationResult = await validator.validateCode(buggyCode, 'javascript')
      const repairResult = await validator.repairCode(buggyCode, validationResult, { autoFix: true })
    ...
    test('should handle tool unavailability gracefully', () => {
      expect(() => {
        enforcer.enforceToolUsage(
          { outputType: 'code' },
          { mode: 'tool', toolName: 'nonexistentTool', fallbackBehavior: 'retry' }
        )
      }).toThrow()
    })

    test('should select optimal tool based on content analysis', () => {
      const toolName = enforcer.selectToolForOutput('code', 'Create a React component')
  ...
  describe('StreamingProgressManager', () => {
    ...
    test('should start progress tracking', () => {
      ...
      const tracker = manager.startTracking(executionId, 'testTool', ['initializing', 'processing', 'complete'])
    ...
    test('should update progress correctly', () => {
      ...
      manager.startTracking(executionId, 'testTool', ['initializing', 'processing', 'complete'], mockController)
    ...
    test('should complete tracking successfully', () => {
      ...
      manager.startTracking(executionId, 'testTool', ['initializing', 'complete'], mockController)

      manager.completeTracking(executionId, { result: 'success' })

      expect(mockController.complete).toHaveBeenCalledWith({ result: 'success' })
    ...
    test('should get progress history', () => {
      ...
      manager.updateProgress(executionId, { stage: 'processing', progress: 25, message: 'Step 1' })
      manager.updateProgress(executionId, { stage: 'processing', progress: 75, message: 'Step 2' })
  ...
  describe('ToolAnalyticsService', () => {
    ...
    test('should track tool execution', () => {
      const tracker = analytics.trackExecution('testTool', 'exec-1', { userId: 'user1' })
        ...
        for (let i = 0; i < 10; i++) {
          promises.push(
            new Promise<void>((resolve) => {
              const tracker = analytics.trackExecution(toolName, `concurrent-${i}`)
              setTimeout(() => {
                tracker.complete({
                  success: true,
                  data: { result: i },
  ...
  describe('Integration Tests', () => {
    ...
    test('should handle error scenarios end-to-end', () => {
      ...
      const tracker = toolAnalyticsService.trackExecution('testTool', executionId)
