...
import type { EnhancedToolResult, ToolExecutionContext } from './tools/enhanced-tools'
...
import type { ToolProgressUpdate } from './streaming-progress'
...
export class ToolAnalyticsService {
  ...
  private readonly MAX_PERFORMANCE_DATA_SIZE = 10000 // Prevent unbounded growth
  ...
  getToolMetrics(toolName: string, forceRefresh = false): ToolUsageMetrics {
    ...
    const toolData = this.performanceData.filter(performanceRecord => performanceRecord.toolName === toolName)
    ...
    const successfulExecutions = toolData.filter(performanceRecord => performanceRecord.success).length
    const failedExecutions = toolData.length - successfulExecutions
    const executionTimes = toolData.map(performanceRecord => performanceRecord.executionTime).sort((a, b) => a - b)
    const totalExecutionTime = executionTimes.reduce((sum, time) => sum + time, 0)

    // Calculate error types
    const errorTypes: Record<string, number> = {}
    toolData.filter(performanceRecord => !performanceRecord.success).forEach(performanceRecord => {
    ...
    const metrics: ToolUsageMetrics = {
      ...
      failedExecutions,
      successRate: toolData.length > 0 ? successfulExecutions / toolData.length : 0,
      averageExecutionTime: toolData.length > 0 ? totalExecutionTime / toolData.length : 0,
      medianExecutionTime: executionTimes.length > 0 ? 
        executionTimes[Math.floor(executionTimes.length / 2)] : 0,
      totalExecutionTime,
      errorTypes,
      lastUsed: Math.max(...toolData.map(performanceRecord => performanceRecord.endTime)),
      firstUsed: Math.min(...toolData.map(performanceRecord => performanceRecord.startTime)),
      userDistribution,
      peakUsageHours
    }
  ...
  getAllToolMetrics(): Record<string, ToolUsageMetrics> {
    const toolNames = [...new Set(this.performanceData.map(performanceRecord => performanceRecord.toolName))]
  ...
  getPerformanceTrends(
    ...
    const toolData = this.performanceData.filter(d => 
      d.toolName === toolName &&
    ...
    const trends = Array.from(buckets.entries()).map(([timestamp, data]) => {
  ...
  getHealthOverview(): {
    ...
    const successfulExecutions = this.performanceData.filter(d => d.success).length
    ...
    const totalTime = this.performanceData.reduce((sum, d) => sum + d.executionTime, 0)
    const avgResponseTime = totalExecutions > 0 ? totalTime / totalExecutions : 0

    const toolsCount = new Set(this.performanceData.map(d => d.toolName)).size

    // Top errors
    const errorCounts = new Map<string, number>()
    this.performanceData.filter(d => !d.success).forEach(d => {
  ...
  private sanitizeExecutionData(executionData: ToolPerformanceData): ToolPerformanceData {
    ...
    const sanitized: ToolPerformanceData = {
      ...
      userId: executionData.userId ? this.sanitizeString(executionData.userId) : undefined,
      sessionId: executionData.sessionId ? this.sanitizeString(executionData.sessionId) : undefined,
      ...
      errorType: executionData.errorType ? this.sanitizeString(executionData.errorType) : undefined,
      errorMessage: executionData.errorMessage ? this.sanitizeString(executionData.errorMessage) : undefined,
      ...
      memoryUsage: executionData.memoryUsage ? Math.max(0, executionData.memoryUsage) : undefined,
      cpuUsage: executionData.cpuUsage ? Math.max(0, executionData.cpuUsage) : undefined,
  ...
  private checkAlerts(performanceData: ToolPerformanceData): void {
    ...
    if (metrics.averageExecutionTime > this.config.alertThresholds.avgExecutionTime) {
      this.createAlert({
        type: 'performance',
        toolName: performanceData.toolName,
        severity: 'medium',
    ...
    const recentFailures = this.getRecentFailureStreak(performanceData.toolName)
  ...
  private createAlert(alertData: Omit<ToolAlert, 'id' | 'timestamp' | 'acknowledged'>): void {
  ...
  private getPeriodMilliseconds(period: AggregationPeriod): number {
    switch (period) {
      ...
      case 'month': return 30 * 24 * 60 * 60 * 1000 // Using 30 days as approximation for month period (standard analytics practice)
    }
  }
  ...
  private startPeriodicCleanup(): void {
    ...
    setInterval(() => {
      ...
      this.performanceData = this.performanceData.filter(d => d.startTime > cutoff)
  ...
  private toCsv(data: ToolPerformanceData[]): string {
    ...
    const rows = data.map(row => 
      Object.values(row).map(value => this.escapeCsvValue(value)).join(',')
    )
...
export class ToolExecutionTracker {
  ...
  private inferErrorType(errorMessage?: string): string {
    ...
    if (message.includes('timeout') || message.includes('deadline') || message.includes('timed out')) return 'Timeout'
    if (message.includes('network') || message.includes('fetch') || message.includes('connection') || message.includes('socket')) return 'Network'
    if (message.includes('permission') || message.includes('auth') || message.includes('forbidden') || message.includes('unauthorized')) return 'Authorization'
    if (message.includes('parse') || message.includes('json') || message.includes('syntax') || message.includes('malformed')) return 'Parsing'
    if (message.includes('memory') || message.includes('limit') || message.includes('overflow') || message.includes('heap')) return 'Resource'
    if (message.includes('rate') || message.includes('throttle') || message.includes('quota')) return 'RateLimit'
    if (message.includes('conflict') || message.includes('concurrent') || message.includes('lock')) return 'Concurrency'
    if (message.includes('not found') || message.includes('404') || message.includes('missing')) return 'NotFound'
    if (message.includes('server') || message.includes('500') || message.includes('internal')) return 'ServerError'
...
export const analyticsConfigSchema = z.object({
  ...
  alertThresholds: z.object({
    ...
    avgExecutionTime: z.number().min(100).default(10000),
