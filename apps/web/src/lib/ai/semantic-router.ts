...
import { systemPrompts } from './providers'
...
export class SemanticRouter {
  ...
  async initialize(): Promise<void> {
    ...
    console.log(`Semantic router initialized with ${this.routes.length} routes`)
  }
...
export const chatRouter = new SemanticRouter({
  routes: [
    {
      name: 'code-help',
      description: 'User needs help with coding, debugging, or technical implementation',
      examples: [
        'debug this code',
        'fix this bug',
...
export const domainRouters = {
  ...
  technical: new SemanticRouter({
    routes: [
      {
        name: 'frontend',
        examples: ['React', 'Vue', 'Angular', 'CSS', 'HTML', 'frontend', 'UI', 'component'],
        metadata: { temperature: 0.3 }
      },
      {
        name: 'backend',
        examples: ['API', 'database', 'server', 'Node.js', 'Python', 'backend', 'REST', 'GraphQL'],
        metadata: { temperature: 0.3 }
      },
      {
        name: 'devops',
        examples: ['Docker', 'Kubernetes', 'CI/CD', 'deployment', 'AWS', 'cloud', 'infrastructure'],
  ...
  business: new SemanticRouter({
    routes: [
      {
        name: 'strategy',
        examples: ['business plan', 'strategy', 'market analysis', 'competition', 'growth'],
        metadata: { temperature: 0.6 }
      },
      {
        name: 'finance',
        examples: ['revenue', 'profit', 'budget', 'financial', 'investment', 'ROI'],
        metadata: { temperature: 0.4 }
      },
      {
        name: 'marketing',
        examples: ['marketing', 'campaign', 'audience', 'brand', 'social media', 'SEO'],
