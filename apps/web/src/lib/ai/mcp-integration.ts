...
export class SYMLogMCPClient {
    ...
    switch (config.type) {
      case 'stdio': {
        ...
        if (!allowedCommands.includes(config.command)) {
          throw new Error(`Command '${config.command}' is not in the allowed list`)
        }
        ...
        for (const arg of config.args) {
          if (dangerousChars.test(arg)) {
            throw new Error('Arguments contain potentially dangerous characters')
          }
        }
  ...
  async getTools(name: string): Promise<MCPToolSet> {
    ...
    try {
      ...
      throw new Error(`Failed to retrieve tools: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
  ...
  async disconnect(name: string): Promise<void> {
    ...
    if (client) {
      try {
        ...
        throw new Error(`Failed to disconnect MCP client ${name}: ${error instanceof Error ? error.message : 'Unknown error'}`)
      }
    }
  }
