...
export class StreamingProgressManager {
  ...
  startTracking(
    executionId: string,
    toolName: string,
    stages: ProgressStage[] = ['initializing', 'processing', 'finalizing', 'complete'],
  ...
  updateProgress(
    ...
    // Validate inputs
    if (!executionId || typeof executionId !== 'string') {
      loggingService.error('Invalid executionId for progress update', { executionId })
      return
    }
    
    if (!update || typeof update !== 'object') {
      loggingService.error('Invalid update object for progress update', { executionId, update })
      return
    }
    
    const tracker = this.activeTrackers.get(executionId)
    if (!tracker) {
      loggingService.warn('Progress update for unknown execution', { executionId })
      return
    }
    ...
    // Validate toolName
    if (update.toolName !== undefined && (!update.toolName || typeof update.toolName !== 'string')) {
  ...
  private calculateProgress(tracker: ProgressTracker): number {
    ...
    const stageProgress = this.calculateStageProgress(tracker)
    const completedStageProgress = tracker.totalStages > 0 ? (tracker.completedStages - 1) / tracker.totalStages * 100 : 0
    const currentStageProgress = tracker.totalStages > 0 ? stageProgress / tracker.totalStages : 0
    
    return Math.min(100, Math.max(0, completedStageProgress + currentStageProgress))
  }
  ...
  private calculateEstimatedDuration(stages: ProgressStage[]): number {
    return stages.reduce((total, stage) => total + this.stageEstimates[stage], 0)
  }
  ...
  private getProgressMessage(tracker: ProgressTracker, stageProgress: number): string {
...
export async function executeToolWithProgress<T>(
  toolName: string,
  execution: (progressUpdate: (update: Partial<ToolProgressUpdate>) => void) => Promise<T>,
  options: {
    executionId?: string
  ...
  const stages = options.stages || ['initializing', 'processing', 'finalizing', 'complete']
 ...
 */
export function createProgressStream(executionId: string): ReadableStream<Uint8Array> {
  ...
  const stream = new ReadableStream<Uint8Array>({
    start(ctrl) {
      ...
      streamingProgressManager.streamControllers.set(executionId, progressController)
    },
    cancel() {
...
export const progressUpdateSchema = z.object({
  ...
  toolName: z.string(),
  stage: z.enum(['initializing', 'validating', 'processing', 'generating', 'optimizing', 'finalizing', 'complete', 'error']),
...
export const streamingProgressConfigSchema = z.object({
  ...
  progressStages: z.array(z.enum(['initializing', 'validating', 'processing', 'generating', 'optimizing', 'finalizing', 'complete', 'error'])),
...
export type StreamingProgressConfigType = z.infer<typeof streamingProgressConfigSchema>