...
const configSchema = z.object({
  ...
  aiMaxTokensPerDay: z.number().default(1000000),
  aiTimeout: z.number().default(120000), // 2 minutes
  ...
  initialize(): Config {
    ...
    try {
      const rawConfig = {
        ...
        // Rate Limiting
        rateLimitWindowMs: process.env.RATE_LIMIT_WINDOW_MS ? parseInt(process.env.RATE_LIMIT_WINDOW_MS) : 60000,
        rateLimitMaxRequests: process.env.RATE_LIMIT_MAX_REQUESTS ? parseInt(process.env.RATE_LIMIT_MAX_REQUESTS) : 100,
        
        // AI Settings
        aiMaxTokensPerRequest: process.env.AI_MAX_TOKENS_PER_REQUEST ? parseInt(process.env.AI_MAX_TOKENS_PER_REQUEST) : 2000,
        aiMaxTokensPerDay: process.env.AI_MAX_TOKENS_PER_DAY ? parseInt(process.env.AI_MAX_TOKENS_PER_DAY) : 1000000,
        aiTimeout: process.env.AI_TIMEOUT ? parseInt(process.env.AI_TIMEOUT) : 120000,
