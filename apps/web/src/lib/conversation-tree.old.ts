...
export class ConversationTreeManager {
  ...
  private createBranchFromEdit(nodeId: string, newContent: string): string {
    ...
    this.createBranch(
      newNodeId,
      `Edit of "${this.getMessagePreview(originalNode.message)}"`
    );
    ...
    if (strategy === 'append') {
      ...
      this.emit({
        ...
        data: {
          sourceBranchId,
          strategy,
          mergedNodes: sourceUniqueNodes.length,
        },
      });
  ...
  deleteMessage(nodeId: string, preserveBranches = false): void {
    ...
    if (affectedBranches.length > 0 && !preserveBranches) {
      throw new Error(
        `Cannot delete node ${nodeId}: it affects ${affectedBranches.length} branches. Use preserveBranches=true to force deletion.`
      );
    }
    ...
    for (const branch of affectedBranches) {
      if (branch.leafNodeId === nodeId) {
        ...
        branch.leafNodeId =
          node.parentId || node.children[0] || branch.rootNodeId;
        branch.messageCount--;
      }
    }
    ...
    if (this.tree.currentNodeId === nodeId) {
      this.tree.currentNodeId =
        node.parentId || node.children[0] || this.tree.rootId;
      this.updateCurrentPath();
    }
  ...
  createBranchFromMessage(nodeId: string, branchName?: string): string {
    ...
    return this.createBranch(
      nodeId,
      branchName || `Branch from "${this.getMessagePreview(node.message)}"`
    );
  }
  ...
  private removeUniqueBranchNodes(branch: Branch): void {
    ...
    for (const nodeId of branchPath) {
      ...
      const otherBranches = this.tree.branches.filter(
        (b) =>
          b.id !== branch.id &&
