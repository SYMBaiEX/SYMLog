...
export class BranchOperations {
  ...
  compareBranches(branchAId: string, branchBId: string): BranchComparison {
    ...
    const commonAncestorId = commonAncestorIndex > 0 
      ? pathA[commonAncestorIndex - 1] 
      : undefined
  ...
  mergeBranches(
    ...
    if (strategy === 'append') {
      const sourcePath = this.navigationManager.getPathToNode(sourceBranch.leafNodeId)
      const targetPath = this.navigationManager.getPathToNode(targetBranch.leafNodeId)
  ...
  getBranchPoints(): Array<{ nodeId: string; branchName?: string; messagePreview: string }> {
