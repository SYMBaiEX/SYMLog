...
export class ConversationTreeManager {
  ...
  mergeBranches(
    ...
    return this.branchOps.mergeBranches(sourceBranchId, targetBranchId, strategy)
  }
  ...
  getStatistics(): {
    ...
    const assistantMessages = nodes.filter(n => n.message.role === 'assistant').length
    ...
    const avgBranchLength = this.tree.branches.length > 0
