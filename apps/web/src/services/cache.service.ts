...
export class CacheService {
  ...
  static clearUserCache(userId: string) {
    this.clearPattern(`/api/user/${userId}`)
  }

  // Clear conversation cache
  static clearConversationCache(conversationId: string) {
    mutate(`/api/conversation/${conversationId}`, undefined, { revalidate: false })
  }

  // Preload conversation data
  static async preloadConversation(conversationId: string) {
    try {
      return mutate(`/api/conversation/${conversationId}`, fetch(`/api/conversation/${conversationId}`).then(r => r.json()))
    } catch (error) {
