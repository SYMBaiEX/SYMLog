...
import { type SchemaType, type SchemaData, schemaRegistry, validateStructuredData } from '@/lib/ai/structured-output'
import { z } from 'zod'
  ...
  const generate = useCallback(async (
    ...
    try {
      ...
      if (stream) {
            ...
            for (const line of lines) {
              try {
                ...
                if (data.type === 'partial') {
                  ...
                  // Validate final data
                  const validationResult = validateStructuredData(data.object, schema)
        ...
        // Validate the result
        const validationResult = validateStructuredData(result.object, schema)
        if (!validationResult.success) {
          throw new Error(`Validation failed: ${validationResult.error}`)
        }
  ...
  const generate = useCallback(async (
    ...
    try {
      ...
      setState(prev => ({
        ...prev,
        isLoading: false,
        error: 'Custom schemas not yet supported. Please use a predefined schema.'
      }))
    } catch (error) {
      setState(prev => ({
        ...prev,
...
export function useStructuredArray<T extends SchemaType>(schemaName: T) {
  ...
  const generateArray = useCallback(async (
    ...
    try {
      ...
      const endpoint = stream ? '/api/ai/stream-object' : '/api/ai/generate-object'
