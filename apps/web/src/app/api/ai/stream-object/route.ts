...
import { streamStructuredData, streamStructuredArray, schemaRegistry, type SchemaType } from '@/lib/ai/structured-output'
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!validationResult.success) {
      ...
      return new Response(`Invalid request: ${validationResult.error.errors[0]?.message}`, { status: 400 })
    }

    const { prompt, schema, model, temperature, maxTokens, mode, output, arrayCount } = validationResult.data
          ...
          try {
            ...
            if (error instanceof NoObjectGeneratedError) {
              const errorChunk = JSON.stringify({
                type: 'error',
                error: 'Failed to generate object: No valid object was generated'
              }) + '\n'
