...
import { generateStructuredData, generateBySchemaName, validateStructuredData, schemaRegistry, type SchemaType, type OutputStrategy } from '@/lib/ai/structured-output'
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { prompt, schema, model, temperature, maxTokens, responseFormat, outputType } = validationResult.data
    ...
    try {
      ...
      if (!(schema in schemaRegistry)) {
        return new Response(`Unknown schema: ${schema}. Available schemas: ${Object.keys(schemaRegistry).join(', ')}`, { status: 400 })
      }

      // Add request timeout to prevent indefinite waiting
      const timeoutPromise = new Promise((_, reject) => {
        setTimeout(() => reject(new Error('Request timeout')), 25000) // 25 seconds, less than maxDuration
      })
      ...
      if (error instanceof NoObjectGeneratedError) {
        logSecurityEvent({
          ...
          metadata: { 
            ...
            prompt: prompt.substring(0, MAX_LOGGED_PROMPT_LENGTH) + (prompt.length > MAX_LOGGED_PROMPT_LENGTH ? '...' : '')
          },
          ...extractClientInfo(req)
        })
        return new Response('Failed to generate object: No valid object was generated', { status: 422 })
      }
