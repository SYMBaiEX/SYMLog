...
import { toolChoiceEnforcer, enforceStructuredOutput, selectOptimalTool } from '@/lib/ai/tool-choice'
import { streamingProgressManager, createProgressStream } from '@/lib/ai/streaming-progress'
import { toolAnalyticsService } from '@/lib/ai/tool-analytics'
import { v2ErrorHandler, errorRecoveryService } from '@/lib/ai/v2-error-handling'
import { getModelWithMetadata, collectResponseMetadata } from '@/lib/ai/providers'
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (requireTools || shouldEnforceTools(lastMessage)) {
      ...
      enhancedToolChoice = enforceStructuredOutput(outputType as any, 'moderate')
          ...
          fallbacks: [
            ...
            async () => {
              logSecurityEvent({
                ...
                userId: userSession.userId,
                metadata: { reason: 'Primary request failed, attempting without tools' },
                ...extractClientInfo(req)
              })
              return await chatService.processChat(
                { 
    ...
    if (handledError.code === 'RATE_LIMIT' || handledError.code === 'MODEL_OVERLOADED') {
      logSecurityEvent({
        type: 'RATE_LIMIT_EXCEEDED',
        metadata: { 
          service: 'ai_provider',
...
function shouldEnforceTools(message: string): boolean {
  ...
  const structuredKeywords = [
    ...
    'chart', 'graph', 'table', 'code', 'function',
    'document', 'report', 'format', 'visualize'
  ]
  ...
  return keywordCount >= 2 || 
         ...
         (lowerMessage.includes('create') && lowerMessage.length > 50) // Long create requests
}
...
function shouldSuggestTool(message: string): boolean {
  ...
  return lowerMessage.includes('show me') ||
