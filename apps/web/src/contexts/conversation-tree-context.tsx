...
interface ConversationTreeActions {
  ...
  addMessage: (message: UIMessage, attachments?: FileAttachment[], parentId?: string, metadata?: Partial<BranchMetadata>) => string
  editMessage: (nodeId: string, newContent: string, createBranch?: boolean) => string
  ...
  mergeBranches: (sourceBranchId: string, targetBranchId: string, strategy?: 'append' | 'replace' | 'manual') => string
  deleteMessage: (nodeId: string, preserveBranches?: boolean) => void
  createBranchFromMessage: (nodeId: string, branchName?: string) => string
}

type TreeAction = 
  | { type: 'INITIALIZE_TREE'; payload: { treeManager: ConversationTreeManager; tree: ConversationTree } }
  | { type: 'UPDATE_TREE'; payload: { tree: ConversationTree } }
  | { type: 'UPDATE_NAVIGATION'; payload: { navigationState: TreeNavigationState } }
...
function treeReducer(state: ConversationTreeState, action: TreeAction): ConversationTreeState {
  switch (action.type) {
    ...
    case 'UPDATE_TREE':
      return {
        ...state,
        currentTree: action.payload.tree,
        navigationState: state.treeManager?.getNavigationState() || state.navigationState
      }
...
export function ConversationTreeProvider({ children, conversationId }: ConversationTreeProviderProps) {
  ...
  const initializeTree = useCallback((id?: string) => {
    ...
    try {
      ...
      const unsubscribe = treeManager.addEventListener((change: TreeStateChange) => {
      ...
      dispatch({ 
        type: 'SET_ERROR', 
        payload: { error: error instanceof Error ? error.message : 'Failed to initialize tree' } 
      })
    }
  }, [])
  ...
  const addMessage = useCallback((
    ...
    return state.treeManager.addMessage(message, attachments, parentId, metadata)
  }, [state.treeManager])

  const editMessage = useCallback((nodeId: string, newContent: string, createBranch = true): string => {
    if (!state.treeManager) throw new Error('Tree manager not initialized')
    return state.treeManager.editMessage(nodeId, newContent, createBranch)
  }, [state.treeManager])

  const regenerateMessage = useCallback((nodeId: string, options?: { model?: string; temperature?: number }): string => {
    if (!state.treeManager) throw new Error('Tree manager not initialized')
    return state.treeManager.regenerateMessage(nodeId, options)
  }, [state.treeManager])

  const createBranch = useCallback((fromNodeId: string, branchName?: string): string => {
    if (!state.treeManager) throw new Error('Tree manager not initialized')
    return state.treeManager.createBranch(fromNodeId, branchName)
  }, [state.treeManager])

  const switchToBranch = useCallback((branchId: string): void => {
    if (!state.treeManager) throw new Error('Tree manager not initialized')
    state.treeManager.switchToBranch(branchId)
  }, [state.treeManager])

  const deleteBranch = useCallback((branchId: string): void => {
    if (!state.treeManager) throw new Error('Tree manager not initialized')
  ...
  // New branch management actions
  const renameBranch = useCallback((branchId: string, newName: string): void => {
    if (!state.treeManager) throw new Error('Tree manager not initialized')
    state.treeManager.renameBranch(branchId, newName)
  }, [state.treeManager])

  const toggleBranchFavorite = useCallback((branchId: string): boolean => {
    if (!state.treeManager) throw new Error('Tree manager not initialized')
    return state.treeManager.toggleBranchFavorite(branchId)
  }, [state.treeManager])

  const setBranchColor = useCallback((branchId: string, color: string): void => {
    if (!state.treeManager) throw new Error('Tree manager not initialized')
    state.treeManager.setBranchColor(branchId, color)
  }, [state.treeManager])

  const compareBranches = useCallback((branchAId: string, branchBId: string): BranchComparison => {
    if (!state.treeManager) throw new Error('Tree manager not initialized')
    return state.treeManager.compareBranches(branchAId, branchBId)
  }, [state.treeManager])

  const mergeBranches = useCallback((sourceBranchId: string, targetBranchId: string, strategy: 'append' | 'replace' | 'manual' = 'append'): string => {
    if (!state.treeManager) throw new Error('Tree manager not initialized')
    return state.treeManager.mergeBranches(sourceBranchId, targetBranchId, strategy)
  }, [state.treeManager])

  const deleteMessage = useCallback((nodeId: string, preserveBranches = false): void => {
    if (!state.treeManager) throw new Error('Tree manager not initialized')
    state.treeManager.deleteMessage(nodeId, preserveBranches)
  }, [state.treeManager])

  const createBranchFromMessage = useCallback((nodeId: string, branchName?: string): string => {
    if (!state.treeManager) throw new Error('Tree manager not initialized')
...
export function useConversationTree() {
  ...
  if (!context) {
    throw new Error('useConversationTree must be used within a ConversationTreeProvider')
  }
  return context
}
...
export function useCurrentTree() {
  ...
  return {
    ...
    nodes: state.currentTree?.nodes ? Array.from(state.currentTree.nodes.values()) : [],
