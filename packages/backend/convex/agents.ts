...
export const getAgentKnowledgeStats = query({
  ...
  handler: async (ctx, args) => {
    ...
    const memoriesByType = memories.reduce((acc, item) => {
    ...
    const recentMemories = memories.filter(m => 
      m.timestamp > Date.now() - 7 * 24 * 60 * 60 * 1000 // Last 7 days
    ).length;
...
export const getAgentLearningProgress = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Group events by day
    const eventsByDay = learningEvents.reduce((acc, event) => {
...
export const searchAgentData = query({
  args: { 
    ...
    dataType: v.optional(v.union(v.literal("memories"), v.literal("knowledge"), v.literal("all")))
  },
  handler: async (ctx, args) => {
    // Verify user owns the agent
    const agent = await ctx.db.get(args.agentId);
    ...
    if (searchMemories) {
      ...
      results.memories = memories.filter(memory => 
        memory.content.toLowerCase().includes(query) ||
        (memory.metadata?.tags && memory.metadata.tags.some(tag => tag.toLowerCase().includes(query)))
      );
    }
